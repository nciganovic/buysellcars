        ...
        <div class="d-flex justify-content-center">
            <div class="col-8">
                <form method="POST" enctype="multipart/form-data"
                    @if($action == "Create")
                        action="{{ route("post_create_user_ad") }}"
                    @else 
                        action="{{ route("post_edit_user_ad", ["id" => $model->id]) }}"
                    @endif>
                    @csrf
                    ...
                    <div class="form-group mt-3">
                        ...
                        <select name="car_id" class="form-select" aria-label="Default select example">
                            ...
                            @foreach ($cars as $c)
                                @if(old('car_id') != null)
                                    <option @if($c->id == old('car_id')) selected @endif value="{{ $c->id }}">{{ $c->brand_name }} {{ $c->car_model_name }}</option>
                                @else 
                                    <option @if($c->id == $model->car_id) selected @endif value="{{ $c->id }}">{{ $c->brand_name }} {{ $c->car_model_name }}</option>
                                @endif               
                            @endforeach
                        </select>
                        @error('car_id')
